...
public class Scoreinmenu : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            TextPrefab[Prefabs] = Instantiate(prefabForCreate, ScoreParent.transform.localPosition,ScoreParent.transform.rotation);
            TextPrefab[Prefabs].transform.SetParent(ScoreParent);
            TextPrefab[Prefabs].transform.localScale = new Vector3(1,1,1);
            TextPrefab[Prefabs].transform.localPosition = new Vector3(TextPrefab[Prefabs].transform.position.x,
                TextPrefab[Prefabs].transform.position.y, -55);
